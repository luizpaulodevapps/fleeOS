...
import { Unlink, X, FileCheck, ClipboardList } from "lucide-react";
import { ReturnFormData } from "../_lib/types";
...
interface ReturnModalProps {
  ...
  closePhotos: Record<string, string>;
  setClosePhotos: React.Dispatch<React.SetStateAction<Record<string, string>>>;
  closeSignature: string;
  setCloseSignature: (sig: string) => void;
  onMockPhotos: () => void;
  loading?: boolean;
}
      ...
      <div className="w-full max-w-lg bg-background border border-outline-variant rounded-xl shadow-2xl relative max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-outline-variant flex justify-between items-center">
          <div>
            <h3 className="text-lg font-bold text-primary font-geist flex items-center gap-1.5">
              ...
              <span>Recolhimento de Veículo</span>
            </h3>
            <p className="text-xs text-on-surface-variant mt-1">Inspeção de devolução e destino físico do ativo.</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-5 text-xs">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-outline mb-2">KM Final do Carro</label>
              <input
                type="number"
                required
                placeholder="Digitar odômetro atual..."
                value={formData.mileageEnd}
                onChange={(e) => setFormData(prev => ({ ...prev, mileageEnd: e.target.value }))}
                className="w-full px-4 py-2 bg-surface-container-low border border-outline-variant rounded-lg text-xs text-on-surface outline-none font-semibold"
              />
            </div>

            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-outline mb-2">Destino do Veículo</label>
              <select
                required
                value={formData.vehicleStatusAfter}
                onChange={(e) => setFormData(prev => ({ ...prev, vehicleStatusAfter: e.target.value as any }))}
              ...
              >
                <option value="active">Frota Ativa (Livre para locação)</option>
                <option value="maintenance">Oficina / Revisão</option>
                <option value="sinistrado">Parado por Sinistro / Danos</option>
              </select>
            </div>
          </div>

          {/* Devolution Checklist */}
          <div className="bg-slate-50 p-4 border border-outline-variant/60 rounded-xl space-y-3">
            <p className="font-bold text-primary flex items-center gap-1.5 uppercase text-[10px] tracking-wider border-b border-outline-variant/40 pb-2">
              ...
              <span>Vistoria de Devolução Obrigatória</span>
            </p>
          ...
          <div className="bg-slate-50 p-4 border border-outline-variant/60 rounded-xl space-y-3">
            <div className="flex justify-between items-center border-b border-outline-variant/40 pb-2">
              <p className="font-bold text-primary flex items-center gap-1.5 uppercase text-[10px] tracking-wider font-geist">
                ...
                <span>Fotos de Evidência da Devolução</span>
              </p>
              <button
                type="button"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {(Object.keys(PHOTO_LABELS) as Array<keyof typeof PHOTO_LABELS>).map((key) => {
                const photoVal = closePhotos[key];
                return (
                  <div key={key} className="relative border border-outline-variant rounded-lg p-1 bg-white hover:bg-slate-50 transition-all flex flex-col items-center justify-center min-h-[72px] text-center">
                    {photoVal ? (
                      <div className="relative w-full h-full min-h-[64px] flex items-center justify-center overflow-hidden rounded">
                        ...
                        <button
                          type="button"
                          onClick={() => setClosePhotos(prev => ({ ...prev, [key]: "" }))}
                        ...
                        <input
                          ...
                          onChange={async (e) => {
                            ...
                            if (file) {
                              ...
                              setClosePhotos(prev => ({ ...prev, [key]: compressed }));
                            }
                          }}
                          className="hidden"
                        />
          ...
          <div>
            ...
            <input
              type="text"
              required
              placeholder="Digitar nome de devolução..."
            ...
            <SignaturePad
              onSave={setCloseSignature}
              onClear={() => setCloseSignature("")}
          ...
          <div className="flex justify-end space-x-3 pt-3 border-t border-outline-variant">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-2 rounded-lg bg-red-650 text-white text-xs font-bold disabled:opacity-50"
            >
              {loading ? "Processando..." : "Concluir Devolução"}
            </button>
          </div>
        </form>
      </div>
