    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-[11px] text-on-surface-variant">Configure os valores padrão diários, semanais e mensais para cada categoria.</p>
        </div>
        <div className="flex gap-2">
          ...
          <button
            onClick={() => {
              setRateForm({ tableId: tables[0]?.id || "", categoryId: categories[0]?.id || "", billingFrequency: "daily", amount: 150 });
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead className="bg-slate-50 border-b border-outline-variant">
            <tr>
              ...
              <th className="px-6 py-3.5 font-semibold text-on-surface-variant uppercase">Categoria</th>
      ...
      {isRateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm">
          <form onSubmit={handleSaveRate} className="bg-white border border-outline-variant rounded-2xl shadow-xl p-6 max-w-sm w-full space-y-4">
            <h3 className="text-sm font-black text-primary font-geist">Configurar Preço da Categoria</h3>
            <div className="space-y-3">
              <div className="floating-label-group">
                <select
              ...
              <div className="floating-label-group">
                <select
                  ...
                  onChange={(e) => setRateForm(prev => ({ ...prev, categoryId: e.target.value }))}
                  className="w-full pl-3 pr-3 text-xs"
                  required
                >
